...
ARG AIRFLOW_DEPS=""
ARG PYTHON_DEPS=""
ARG AIRFLOW_EXTRAS="postgres,jdbc,mysql,google,google_auth,cncf.kubernetes,ssh,sftp,statsd"
...
COPY scripts/set_connections.sh ${AIRFLOW_USER_HOME}/set_connections.sh

RUN chmod +x ${AIRFLOW_USER_HOME}/set_connections.sh
